...
if (!target || !destination) throw new Error("Set target URL and fresh recording directory");
const check = process.argv.includes("--check");
await mkdir(destination, { recursive: false });
const browser = await chromium.launch();
const context = await browser.newContext({
  viewport: { width: 1440, height: 1000 },
  ...(check ? {} : { recordVideo: { dir: destination, size: { width: 1440, height: 1000 } } }),
});
...
try {
  ...
  await page.locator("#idea").pressSequentially("Investigate the original monthly score strategy", { delay: check ? 0 : 55 });
  await expect(page.locator("#idea")).toHaveValue("Investigate the original monthly score strategy");
  ...
  for (const [index, heading, content] of [
    ...
    [4, "The execution guard stops the attempt.", "MONTHLY_BORROW_UNAVAILABLE_OR_AMBIGUOUS"],
    ...
    await page.screenshot({ path: path.join(destination, `0${index}-scene.png`), fullPage: true });
    scenes.push(heading);
    if (!check) await page.waitForTimeout(index === 1 || index === 2 ? 9500 : 7500);
  }
  await expect(page.locator("#intro")).toContainText("not a correction by the agent");
  if (check) {
...
await writeFile(path.join(destination, "recording.json"), JSON.stringify({
  url: target, scope: "Real browser recording of retained evidence replay on authored inputs",
  liveInference: false, scenes, pageErrors: errors,
  video: check ? null : await page.video().path(),
}, null, 2));
console.log(JSON.stringify({ status: "passed", scenes: scenes.length, recording: !check, destination }));
