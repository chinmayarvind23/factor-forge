...
Check cache reuse and malformed-locator recovery against an operator-provided Redis
endpoint with `uv run --project tools/cache --locked python tools/cache/verify.py`.
The check uses controlled HTTP metadata.
