"""Budgeted source synthesis preserves prompts, observations and replay identity."""
...
from factorforge.retrieval.synthesis import (
    SYNTHESIS_PROMPT,
    SynthesisRequest,
    SynthesisResult,
    synthesize,
...
class SynthesisCommand(Contract):
    """A captured source, fixed provider profile and prompt identity define one model attempt."""

    schema_version: Literal["source-synthesis-command-v1"] = "source-synthesis-command-v1"
    profile: Literal["ollama-qwen3-8b-source-synthesis-32k-v1"] = (
        "ollama-qwen3-8b-source-synthesis-32k-v1"
    )
...
class SynthesisOperationResult(Contract):
    """The durable result binds the exact worker command to the reviewer's full evidence record."""

    schema_version: Literal["source-synthesis-operation-result-v1"] = (
        "source-synthesis-operation-result-v1"
    )
    command: SynthesisCommand
    synthesis: SynthesisResult


def execute_synthesis_operation(
    ...
    command = SynthesisCommand.model_validate(command)
    if command.prompt_sha256 != PROMPT_SHA256:
        raise ResearchError(
            "SYNTHESIS_PROFILE_UNSUPPORTED",
            "The source synthesis profile is unavailable.",
            409,
        )
    operation = Operation(
        operation_id=uuid5(run_id, "source-synthesis-worker-v1:" + command.sha256),
        kind="llm",
    ...
    runs._inject("after_synthesis_reservation")
    ...
    result = SynthesisOperationResult(command=command, synthesis=review)
    ...
    if len(raw) > 2**20 or artifacts.put(raw, media_type="application/json") != expected:
        raise ResearchError(
            "RESEARCH_RESULT_INVALID", "The source synthesis result cannot be verified.", 409
        )
    runs._inject("before_synthesis_settlement")
    settle_operation(
        runs,
        run_id,
        principal,
...
def _recover(
    artifacts: ArtifactStore, ref: ArtifactRef, command: SynthesisCommand
) -> SynthesisResult:
    ...
    try:
        if ref.size_bytes > 2**20 or ref.media_type != "application/json":
            raise ValueError("Unsupported source synthesis result envelope")
        ...
        parsed = SynthesisOperationResult.model_validate_json(raw)
        if parsed.canonical_bytes() != raw or parsed.command != command:
            raise ValueError("Source synthesis command mismatch")
