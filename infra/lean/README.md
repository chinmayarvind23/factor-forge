...
It supplies the runtime assemblies for the source-driven LEAN adapter.
...
Use the digest rather than a mutable tag.
...
The adapter must supply a local-only configuration and source inputs, enforce a
separate runtime policy, and capture actual LEAN output.
...
child's exit status. Reaching the log cap fails capture.
