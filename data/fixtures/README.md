...
`tiny-market-v1.json` contains original fictional securities and manually chosen values for deterministic data-contract tests. It covers six dates from 29 April through 6 May 2024. It is not observed equity data and cannot support factor-reproduction or investment-performance claims.
...
These timestamps define a test market. They do not model exchange settlement, real regulatory deadlines or a real feed's latency. Formation-time information must still precede trading under the application's strict time-order rule. No trading volume, shares outstanding, trading costs, complete benchmark lineage or published-paper labels are supplied.
